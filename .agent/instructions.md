...
- **ESLint Zero Warnings**: `npm run lint` 실행 시 경고(Warning)가 0개인 상태를 항시 유지하며, 정석적인 코딩 스타일을 지향한다.
- **Prop-Types 필수 정의**: 모든 JSX 컴포넌트는 `prop-types` 라이브러리를 사용하여 모든 props에 대한 타입을 명시적으로 정의해야 한다.
- **명시적 React 임포트**: React 17+ 버전에서도 `import React from 'react';` 구문을 파일 상단에 반드시 포함하여 가독성과 명시성을 높인다.
...
## 개발용 테스트 페이지 규칙 (Dev Sandbox) (중요)

여러 개발자가 동시에 작업할 때 충돌을 방지하고, 각 컴포넌트를 독립적으로 테스트하기 위한 규칙이다.

### App.jsx 사용 제한

- App.jsx에 테스트용 UI나 임시 컴포넌트를 직접 렌더링하지 않는다
- App.jsx는 라우터 연결 및 최상위 구조 역할만 수행한다
- 테스트 목적의 임시 코드는 App.jsx에 작성하지 않는다

---

### Dev 페이지 생성 규칙

- 새로운 공통 컴포넌트 또는 UI 작업 시 Dev 페이지를 반드시 생성한다
- Dev 페이지는 `src/pages/dev` 경로에 생성한다
...
- 컴포넌트 단독 렌더링
- 다양한 props 테스트
- 상태별 UI 확인 (disabled, error, helperText 등)
- 스타일 및 레이아웃 확인
...
- Dev 페이지는 테스트 목적 코드만 포함한다
- 비즈니스 로직을 포함하지 않는다
- 실제 서비스 로직과 분리한다
- 하나의 Dev 페이지는 하나의 컴포넌트 테스트를 원칙으로 한다

---

### PR 규칙

- 새로운 공통 컴포넌트 추가 시 Dev 페이지도 함께 포함하는 것을 권장한다
- App.jsx에 테스트 코드가 포함된 PR은 반려 대상이다

---

### AI 작업 규칙

- AI는 테스트 코드를 App.jsx에 추가하지 않는다
- 새로운 컴포넌트 생성 시 Dev 페이지 생성도 함께 고려한다
- 수정 범위가 불명확할 경우 Dev 페이지 사용을 우선 제안한다
